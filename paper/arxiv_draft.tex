...
includes calibrating the detector on full-match footage, fitting the xT
surface per cross-validation fold (currently league-wide, a negligible leakage
given its aggregate nature), and tracking-derived possession features are now in place too;
the feature scaler, calibrated classifier, repeated cross-validated evaluation,
per-90/lineup-aware player features, the modality-blocklist rights gate, an open
event-data model with expected threat, and player-match/player-form graph
payloads are already in place.
