...
home and away scores, using the five detection-derived features above.
...
\begin{table}[h]
\centering
\caption{In-sample results on the 98-match highlight dataset.
Accuracy is reported against the majority-class baseline to expose the
limited predictive content of highlight-clip detection aggregates.}
\label{tab:results}
\begin{tabular}{lcc}
\toprule
...
\paragraph{Honest limitation.} As Table~\ref{tab:results} shows, the highlight
detection features do \emph{not} outperform the majority-class baseline for
winner prediction. This is the expected and, we argue, informative result:
short highlight reels are highlight-selected (not representative of open
play), contain far fewer frames than a full match, and aggregate detection
counts are nearly invariant to final score. The pipeline therefore demonstrates
end-to-end reproducibility and governance, while clearly delineating where
predictive signal must come from: full-match footage or rich open event data
(Section~\ref{sec:data}). We report in-sample figures only and do not claim
generalization.

\section{Reproducibility}
The study is scripted and re-runnable: \texttt{scripts/process\_highlights\_batch.py}
(transcode + detect, resumable) and \texttt{scripts/build\_highlights\_training.py}
(aggregate + fine-tune). A data card and a zipped bundle of the processed tables
...
includes calibrating the detector on full-match footage, adding a feature
scaler for the classifier, and incorporating open event features (e.g.,
xT, pressure) as stronger supervisory signal.
