...
open event data, non-audiovisual). Using home/away event-derived features
(expected goals, shots, shots on target, passes, pressures, carries, clearances,
interceptions, blocks, fouls, ball recoveries) and repeated
5-fold $\times$ 10 cross-validation, the winner classifier reaches
\textbf{63.3\% ($\pm$2.9)} accuracy against a \textbf{41.3\%} majority-class
baseline, with a calibrated Brier score of \textbf{0.164} and home/away score
MSE of \textbf{0.96 / 0.87}. This confirms the pipeline yields genuine,
calibrated lift once fed rights-clean open supervision---precisely the signal
...
includes calibrating the detector on full-match footage and adding richer
event features (e.g., xT, pressure regains) as stronger supervisory signal; the
feature scaler, calibrated classifier, and repeated cross-validated evaluation
are already in place, and an open event-data model is demonstrated above.
